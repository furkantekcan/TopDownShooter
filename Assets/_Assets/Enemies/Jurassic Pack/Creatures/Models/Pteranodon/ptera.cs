...
public class ptera : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		{
			if(shared.IsInWater && shared.Health==0) { body.mass=10; body.linearDamping=1; body.angularDamping=1; }
			else  { body.mass=1; body.linearDamping=4; body.angularDamping=4; }
        ...
        if(CurrAnm.IsName("Ptera|Fall")) anm.SetBool("OnGround", true);
			...
			else
			{
				...
				if(shared.IsOnWater) { body.mass=1; body.linearDamping=1; body.angularDamping=1; } else
				{ 
					body.mass=1; body.linearDamping=4; body.angularDamping=4; flyRoll = Mathf.Lerp(flyRoll, 0.0f, 0.1f); flyPitch = Mathf.Lerp(flyPitch, 0.0f, 0.1f);
					body.AddForce(Vector3.up*(shared.posY-transform.position.y)*8); shared.Health-=0.01f;
				}
			}
		} else { body.mass=1; body.linearDamping=1; body.angularDamping=1; }  //In air
			...
			CurrAnm.IsName("Ptera|Die1") | CurrAnm.IsName("Ptera|Die2") | CurrAnm.IsName("Ptera|Fall"))
		{
			...
			{
				reset=true; shared.IsConstrained=true; body.linearVelocity = new Vector3(0, 0, 0); 
		   ...
		   NextAnm.IsName("Ptera|Glide") | CurrAnm.IsName("Ptera|Glide") | NextAnm.IsName("Ptera|GlideGrowl") | CurrAnm.IsName("Ptera|GlideGrowl"))
		{
			...
			if(body.linearVelocity.magnitude!=0) body.AddForce(-Vector3.up*Mathf.Lerp(dir.y, (64/body.linearVelocity.magnitude)*transform.localScale.x, 1.0f));
		...
		if(shared.OnLevitation)
		{
			...
			source[2].volume=body.linearVelocity.magnitude/(80*transform.localScale.x);
			source[2].pitch=body.linearVelocity.magnitude/(40*transform.localScale.x);
		}
