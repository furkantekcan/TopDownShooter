...
public class dilo : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		//Set mass
		if(shared.IsInWater) { body.mass=10; body.linearDamping=1; body.angularDamping=1; }
			...
			CurrAnm.IsName("Dilo|IdleJumpEnd") | CurrAnm.IsName("Dilo|RunJumpEnd") | CurrAnm.IsName("Dilo|JumpAtk"))
		{
			...
			{ 
				body.linearDamping=4; body.linearVelocity = new Vector3(body.linearVelocity.x, 0, body.linearVelocity.z);
			...
			else { body.linearDamping=0.1f; if(CurrAnm.IsName("Dilo|JumpAtk")) { shared.IsAttacking=true; PlaySound("AtkB", 1); PlaySound("Bite", 9); } }
		}
